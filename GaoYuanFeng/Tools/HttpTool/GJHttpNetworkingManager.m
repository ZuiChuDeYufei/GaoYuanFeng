...
- (instancetype)init
{
    self = [super init];
    if (self) {
        _httpConditionType = kHttpCondition_Distribution;
    }
    return self;
}
...
- (void)reqeustLoadImageData:(NSData *)data
                         url:(NSString *)url
...
- (NSDictionary *)configFullParameter:(NSDictionary *)parameter {
    ...
    // appid
    [fullParaDic addEntriesFromDictionary:@{@"appid":[[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleIdentifier"]}];
    return fullParaDic;
}
