...
#define Tag_InputField  34657674

@interface GJModifyNicknameVC () <UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate>
@property (nonatomic, strong) UITableView *tableview;
@property (nonatomic, strong) GJMineManager *mineManager;

@end

@implementation GJModifyNicknameVC
...
- (void)requestModifyNickName:(NSString *)nickName {
    _mineManager = [[GJMineManager alloc] init];
    [_mineManager requestEditUserInfoWithNick:nickName context:self];
}
    ...
    @try {
        ...
        if (tf.text.length >= 2 && tf.text.length <= 16) {
            if ([tf.text isValidNickName]) {
                [self requestModifyNickName:tf.text];
            }
            else ShowWaringAlertHUD(@"昵称格式有误，请重新输入", nil);
        }else {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        nickNameTF.text= APP_USER.userInfo.nickname;
